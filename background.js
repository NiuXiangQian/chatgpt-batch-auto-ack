async function handleProcessQuestion(e, t, a) {
    try {
        const t = void 0 === e.useTempChat || e.useTempChat ? "https://chatgpt.com/?temporary-chat=true" : "https://chatgpt.com/",
            r = await chrome.tabs.query({url: "https://chatgpt.com/*"});
        let s;
        r.length > 0 ? (s = r[0], await chrome.tabs.update(s.id, {
            url: t,
            active: !0
        }), await sleep(2e3)) : (s = await chrome.tabs.create({url: t, active: !0}), await waitForTabLoad(s.id));
...
async function handleOpenChatGPT(e, t) {
    try {
        const a = void 0 === e.useTempChat || e.useTempChat ? "https://chatgpt.com/?temporary-chat=true" : "https://chatgpt.com/",
            r = await chrome.tabs.query({url: "https://chatgpt.com/*"});
        if (r.length > 0) await chrome.tabs.update(r[0].id, {url: a, active: !0}), t({
            success: !0,
            tabId: r[0].id
        }); else {
            t({success: !0, tabId: (await chrome.tabs.create({url: a, active: !0})).id})
        }
    } catch (e) {
